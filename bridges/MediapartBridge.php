...
//
		$action = 'https://www.mediapart.fr'.str_replace('http://','https://',$auth->action);
//
		$post_data['name']          = $user;
...
//		$post_data['form_id']       = $auth->find('input[name=form_id]', 0)->value;
		...
		$parse_header = function($ch, $headline) use(&$cookie, &$httplocation) {
			...
			if(preg_match('/^Set-Cookie:\s*([^;]+).*(?:;\s*expires=([^;]*)).*$/i', $headline, $matches)) {
				...
				if(preg_match('/^MPSESSID.*$/i', $matches[1]))
					$cookie->expires = strtotime($matches[2]);
...
// temporaire en attendant de trouver meilleure solution
...
/////////////// FAUDRA REMETTRE UNE LOGIQUE
		...
		// 01 - deletion of "Lire aussi"
		$lireaussi = $html->find('div.content-article div.read-also');
...
//if($html->find('div.introduction', 0) )  echo "ok"; else echo "NO";
//echo $html->plaintext;

//echo $url;
//echo file_get_html($url.'?onglet=full', false, $session)->plaintext;
...
//
				$item->uri       = str_replace(['http://','/commentaires'], ['https://',''], $item->uri);
//
